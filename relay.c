...
const CliCmdType CMD_HELP =
{
	...
	"\tUsage:      4relay -h    Display command options list\n",
	"\tUsage:      4relay -h <param>   Display help for <param> command option\n",
	"\tExample:    4relay -h write    Display help for \"write\" command option\n"};
...
const CliCmdType CMD_VERSION =
{
	...
	"\tUsage:          4relay -v\n",
	"",
	"\tExample:        4relay -v  Display the version number\n"};
...
const CliCmdType CMD_WAR =
{
	...
	"\tUsage:          4relay -warranty\n",
	"",
	"\tExample:        4relay -warranty  Display the warranty text\n"};
...
const CliCmdType CMD_LIST =
{
	...
	1,
	&doList,
	"\t-list:       List all 4relay boards connected,\n\treturn       nr of boards and stack level for every board\n",
	"\tUsage:       4relay -list\n",
	"",
	"\tExample:     4relay -list display: 1,0 \n"};
...
const CliCmdType CMD_WRITE =
{
	...
	"\tUsage:       4relay <id> write <channel> <on/off>\n",
	"\tUsage:       4relay <id> write <value>\n",
	"\tExample:     4relay 0 write 2 On; Set Relay #2 on Board #0 On\n"};
...
const CliCmdType CMD_READ =
{
	...
	"\tUsage:       4relay <id> read <channel>\n",
	"\tUsage:       4relay <id> read\n",
	"\tExample:     4relay 0 read 2; Read Status of Relay #2 on Board #0\n"};
...
const CliCmdType CMD_TEST =
{
	...
	"",
	"\tUsage:       4relay <id> test\n",
	"\tExample:     4relay 0 test\n"};

CliCmdType gCmdArray[CMD_ARRAY_SIZE];

char *usage = "Usage:	 4relay -h <command>\n"
	"         4relay -v\n"
	"         4relay -warranty\n"
	"         4relay -list\n"
	"         4relay <id> write <channel> <on/off>\n"
	"         4relay <id> write <value>\n"
	"         4relay <id> read <channel>\n"
	"         4relay <id> read\n"
	"         4relay <id> test\n"
	"Where: <id> = Board level id = 0..7\n"
	"Type 4relay -h <command> for more help"; // No trailing newline needed here.
...
static void doRelayWrite(int argc, char *argv[])
{
	...
	if ( (argc != 5) && (argc != 4))
	{
		printf("Usage: 4relay <id> write <relay number> <on/off> \n");
		printf("Usage: 4relay <id> write <relay reg value> \n");
		exit(1);
	}
...
static void doVersion(int argc, char *argv[])
{
	UNUSED(argc);
	UNUSED(argv);
	printf("4relay v%d.%d.%d Copyright (c) 2016 - 2020 Sequent Microsystems\n",
	VERSION_BASE, VERSION_MAJOR, VERSION_MINOR);
	printf("\nThis is free software with ABSOLUTELY NO WARRANTY.\n");
	printf("For details type: 4relay -warranty\n");
